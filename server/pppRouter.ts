import { countries, pppRules } from "@/db";
...
export const pppRouter = createTRPCRouter({
    ...
    .mutation(async ({ ctx, input }) => {
      for (const code of input.countries) {
        const [country] = await ctx.db.select().from(countries).where(eq(countries.code, code)).limit(1);
        if (!country) throw new TRPCError({ code: "NOT_FOUND", message: `Country ${code} not found` });
      }

      const [rule] = await ctx.db.insert(pppRules).values(input).returning();
      return rule;
    }),
  ...
  updateRule: orgAdminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [rule] = await ctx.db
        ...
        .where(and(eq(pppRules.id, input.id), eq(pppRules.organizationId, input.organizationId)))
        .returning();
  ...
  listRules: orgAccessProcedure
    ...
    .query(async ({ ctx, input }) => {
      return await ctx.db
        .select()
        .from(pppRules)
        .where(eq(pppRules.organizationId, input.organizationId));
    }),
